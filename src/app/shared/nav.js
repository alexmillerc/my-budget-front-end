  ...
  return (
    <AppBar position='static' style={{ background: '#52616b' }}>
      <Toolbar style={{ maxWidth: '1200px', margin: '0 auto' }} className='flex items-center w-100'>
        ...
        <Button variant='contained' style={{ margin:'0 1em 0 0', background: '#f0a500' }} onClick={() => orcamentoNew()}><Add /> Orçamento</Button>
        <Button variant='contained' style={{ margin:'0 1em 0 0', background: '#f0a500' }} onClick={() => saveUserData(dispatch, { email, orcamentos })}><Save /> Salvar</Button>
        <Button variant='contained' style={{ background: '#cf7500' }} onClick={() => logout()}><ArrowBack /> Sair</Button>
      </Toolbar>
    </AppBar>
  );
};